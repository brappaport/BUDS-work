...
data %>%
  mutate(Like_Acc=Acc_Acc,
         ...
         Like_Acc_z = scale(Acc_Acc, center=T, scale=T),
         Dislike_Acc_z = scale(Rej_Acc, center=T, scale=T),
         Like_Rej_z = scale(Acc_Rej, center=T, scale=T),
         Dislike_Rej_z = scale(Rej_Rej, center=T, scale=T)) %>%
...
geocode_data <- read.csv(here("../_rawdata/GeocodeResults.csv"), header=F, colClasses='character') %>%
...
# Import subject ID 
subj_rownumber <- openxlsx::read.xlsx(here("../_rawdata/Address/PREDICT_ids.xlsx"))
...
# geocode_data_with_adi[geocode_data_with_adi$ID==9027,"ADI_NATRANK"] MISSING ADDRESS, but sibling of 9026 so see below
geocode_data_with_adi[geocode_data_with_adi$ID==9039,"ADI_NATRANK"] <- 44 # previously 28 but on college campus
...
geocode_data_with_adi[geocode_data_with_adi$ID==9056,"ADI_NATRANK"] <- 7 # previously NA but on college campus
geocode_data_with_adi[geocode_data_with_adi$ID==9059,"ADI_NATRANK"] <- (21+29)/2 # but on college campus
geocode_data_with_adi[geocode_data_with_adi$ID==9060,"ADI_NATRANK"] <- 57 # but on on college campus but previous address was in Mexico
...
geocode_data_with_adi[geocode_data_with_adi$ID==9091,"ADI_NATRANK"] <- 21 # previously 28 but on college campus
geocode_data_with_adi[geocode_data_with_adi$ID==9127,"ADI_NATRANK"] <- 63 # current address is on college campus, this reflects participant family's address before college
geocode_data_with_adi[geocode_data_with_adi$ID==9128,"ADI_NATRANK"] <- 73 # current address is on college campus, this reflects participant family's address before college
...
address[address$ID==9059,"V2"]

# Subjects over 18
load(file=here("./data/over_18_150275.RData"))
load(file=here("./data/over_18_275425.RData"))
over_18 <- address %>%
