---
title: "Data prep script 3: Importing ADI and scoring self-report data"
...
# About
This script imports and cleans ADI (Area Deprivation Index) and scores self-report data from PREDICT project collaboration between Northwestern (PI: Shankman) and Columbia (PI: Auerbach).
...
## Demographics
```{r, eval=TRUE}
demo_vars <- c("id_1_i","demo_child_age","demo_child_gender","demo_child_sex","demo_child_race","demo_child_hispanic","demo_child_sex_or")
...
BUDS_cleaning04_demo <- rbind(cu_demo, nw_demo) %>%
  ...
  dplyr::select(c(ID, demo_child_age, demo_child_gender, demo_child_sex, demo_child_race, demo_child_hispanic, White_nonwhite)) %>%
  filter(complete.cases(demo_child_sex)) %>%
  mutate(demo_child_age = as.numeric(demo_child_age))
...
## IDAS
```{r}
BUDS_cleaning04_idas <- as.data.frame(BUDS_cleaning03) %>%
