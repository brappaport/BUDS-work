---
title: "Data prep script 4: In-task and post-task ratings"
...
# About
This script organizes behavioral ratings during the Social Feedback task for the BUDS study using data from PREDICT project collaboration between Northwestern (PI: Shankman) and Columbia (PI: Auerbach).
...
```{r}
  here::i_am("./management/data04_BUDS_ratings.Rmd")
```
...
save(df_beh, file=here("./data/df_beh.RData"))
```

```{r}
df_BUDS_rating <- df_beh %>%
  mutate(PicRating_n=as.numeric(PicRating)) %>%
  filter(complete.cases(Event) & Event!="none") %>%
  dplyr::select(ID,Event,PicRating_n) %>%
  group_by(ID,Event) %>%
  reframe(rating = mean(PicRating_n)) %>%
  mutate(Condition=case_match(Event, "Accept-Accept" ~ "Acc_Acc",
...
## Post-task ratings
```{r}
df_BUDS_rating_pt_cu <- read.csv(here("../_rawdata/SP_Task_Debriefing_CU.csv"), colClasses = "character", 10) %>%
  mutate(ID = id_1_i) %>%
  dplyr::select(ID, id_2_i, starts_with("SP_debriefing")) %>%
  filter(complete.cases(ID)) %>%
  arrange(ID) %>%
  mutate(SP_debriefing_1 = case_match(SP_debriefing_1, "Very Uninterested =  1" ~ '1',
                                      "Very Interested  =10" ~ '10',
                                      NA ~ NA,
                                      .default = SP_debriefing_1),
         SP_debriefing_2 = case_match(SP_debriefing_2, "Not Very Happy = 10" ~ '1',
                                      "Very Happy = 10" ~ '10',
                                      NA ~ NA,
                                      .default = SP_debriefing_2),
         SP_debriefing_3 = case_match(SP_debriefing_3, "Not Very Upset = 1" ~ '1',
                                      "Very Upset =10" ~ '10',
                                      NA ~ NA,
                                      .default = SP_debriefing_3),
         SP_debriefing_4 = case_match(SP_debriefing_4, "Not at All  = 1" ~ '1',
                                      "Very Angry =10" ~ '10',
                                      NA ~ NA,
                                      .default = SP_debriefing_4),
         SP_debriefing_5 = case_match(SP_debriefing_5, "Not at All = 1" ~ '1',
                                      "Very Nervous =10" ~ '10',
                                      NA ~ NA,
                                      .default = SP_debriefing_5),
         SP_debriefing_6a = case_match(SP_debriefing_6a, "Yes" ~ '1',
                                      "No" ~ '2',
                                      NA ~ NA,
                                      .default = SP_debriefing_6a),
         SP_debriefing_7 = case_match(SP_debriefing_7, "A lot = 10" ~ '10',
                                      NA ~ NA,
                                      .default = SP_debriefing_7)) %>%
      mutate(across(c("ID",contains("SP_debriefing")), as.numeric))
head(df_BUDS_rating_pt_cu)
...
```{r}
load(file=here("./data/BUDS_df_ERP.RData"))

df_50150_cz_rating <- df_50150_cz %>%
  dplyr::select(ID,AA_AR) %>%
  full_join(df_BUDS_rating_it_pt, by="ID", relationship = "many-to-many") %>%
  dplyr::select(ID,site,Condition,rating,AA_AR,SP_debriefing_deceived) %>%
  group_by(ID,site,Condition,AA_AR,SP_debriefing_deceived) %>%
...
# summary(lm(AA_AR ~ rating_r, data = df_50150_cz_rating))
...
# summary(lm(AA_AR ~ rating_r, data = df_150275_cz_rating))

df_275425_cz_rating <- df_275425_cz %>%
  dplyr::select(ID,AA_AR) %>%
  full_join(df_BUDS_rating_it_pt, by="ID", relationship = "many-to-many") %>%
  dplyr::select(ID,site,Condition,rating,AA_AR,SP_debriefing_deceived) %>%
  group_by(ID,site,Condition,AA_AR,SP_debriefing_deceived) %>%
...
# summary(lm(AA_AR ~ rating_r, data = df_275425_cz_rating))
```

# Save files
```{r}
save(df_BUDS_rating, df_BUDS_rating_it_pt, 
     df_50150_cz_rating, df_150275_cz_rating, df_275425_cz_rating, file=here("./data/BUDS_behavior.RData"))
```
