...
```{r}
load(here::here("data/df_beh.RData"))
```
...
```{r}
df_BUDS_rating_it_pt <- df_BUDS_rating_pt %>%
  distinct(ID, .keep_all = TRUE) %>% # remove handful of duplicate rows that were identical
  full_join(df_BUDS_rating_it, by="ID", relationship = "many-to-many")
