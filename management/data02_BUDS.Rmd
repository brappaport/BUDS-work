...
### 2.1.1 Alphas
```{r}
# psych::alpha(BUDS_cleaning03[,c(addi_vars)])$total$std.alpha
# psych::alpha(BUDS_cleaning03[,c(addi_vars[c(1,2,3,6)])])$total$std.alpha
# psych::alpha(BUDS_cleaning03[,c(addi_vars[c(7,9,10,13,14)])])$total$std.alpha
# psych::alpha(BUDS_cleaning03[,c(addi_vars[c(4,5,8,11,15)])])$total$std.alpha
```
...
original <- '
physical =~ bcs_offline_1_i + bcs_offline_2_i + bcs_offline_3_i + bcs_offline_4_i
verbal =~ bcs_offline_5_i + bcs_offline_6_i
relational =~ bcs_offline_7_i + bcs_offline_8_i
'

new <- '
physical =~ bcs_offline_1_i + bcs_offline_4_i
verbal =~ bcs_offline_5_i + bcs_offline_6_i
relational =~ bcs_offline_2_i + bcs_offline_3_i + bcs_offline_7_i + bcs_offline_8_i
'
...
BUDS_cleaning05 <- BUDS_cleaning04 %>% 
    rowwise() %>%
    mutate(promis_peer_meanraw = mean(c_across(promis_peer_rel_1_i:promis_peer_rel_8_i), na.rm=F),
           promis_fam_meanraw = mean(c_across(promis_family_rel_1_i:promis_family_rel_4_i), na.rm=F),
           promis_peer_sumraw = sum(c_across(promis_peer_rel_1_i:promis_peer_rel_8_i), na.rm=F),
           promis_fam_sumraw = sum(c_across(promis_family_rel_1_i:promis_family_rel_4_i), na.rm=F)) %>%
   mutate(
...
### 2.3.1 Alphas
```{r}
psych::alpha(BUDS_cleaning05[,c(promis_vars[c(1:8)])])$total$std.alpha
psych::alpha(BUDS_cleaning05[,c(promis_vars[c(9:12)])])$total$std.alpha
```
...
table(BUDS_cleaning05$promis_fam_sumraw)
...
```
```{r}
all_measures <- c("addi_total","addi_educ","addi_instit","addi_distress",
                  "bcs_total_vic","bcs_physical_vic_01","bcs_verbal_vic","bcs_rel_vic","bcs_cyber_vic",
                  "promis_peer_tscore","promis_fam_tscore")

write.csv(correlation_matrix(BUDS_cleaning05[c(all_measures)], use="lower", replace_diagonal = TRUE, show_significance=FALSE), file = "manuscript/tables/beh_cor_matrix.csv")
