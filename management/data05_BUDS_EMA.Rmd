...
# About
This script cleans EMA data and merges it with ERP and self-report data from PREDICT project collaboration between Northwestern (PI: Shankman) and Columbia (PI: Auerbach).
...
## Load ERP data
```{r}
load(here::here("./data/BUDS_do01.RData"))
```

# EMA (intensive assessments)
```{r}
...
# remove people with < 4 responses
resp.rate = baseline %>% dplyr::count(subject_id)
remove = resp.rate %>% filter(n < 4)
subjects_with_less_than_4_responses <- resp.rate[(resp.rate$n<4),"subject_id"]
write.csv(subjects_with_less_than_4_responses, file=here::here("./data/number_with_less_than_4_EMA_responses.csv"))
...
df_50150_fz_ema <- residualizing(df_50150_fz)
df_150275_cz_ema <- residualizing(df_150275_cz)
df_275425_pz_ema <- residualizing(df_275425_pz)
```

## Clean ema data and calculate NA cubed
```{r}
# Clean ema datasets
...
# Save files
```{r}
save(df_50150_fz, df_50150_fz_ema, 
     df_150275_cz, df_150275_cz_ema,
     df_275425_pz, df_275425_pz_ema, file=here::here("./data/BUDS_do02.RData"))
```
