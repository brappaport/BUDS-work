...
abstract: |
  *Background:* Lower socioeconomic status (SES) is a potent risk factor for psychopathology in youth. Because lower SES youth experience greater prejudice, discrimination, and stigma from others, one neural mechanism that may link SES to psychopathology risk is aberrant brain responding to accepting feedback. This study will test whether SES (operationalized as neighborhood area deprivation) is: 1) related to brain responses to peer feedback, and 2) moderates the relationship between brain responses to peer feedback and affect, measured through experience momentary assessment (EMA). *Methods:* Adolescents (N=172; ages 13-18) were administered an event-related potential (ERP) task during which they received accepting or rejecting feedback from peers to probe their sensitivity to acceptance via the reward positivity (RewP) and P300 ERP components. They additionally completed a week of EMA reporting on their positive and negative affect. Results: Lower SES was related to blunted responses to acceptance (in the RewP and P300, relative to rejection). Interestingly, lower SES moderated relationships between brain responses (RewP and P300) to peer acceptance and negative affect, with adolescents with lower SES exhibiting a more negative relationship between EMA reported negative affect and brain response to acceptance than adolescents with higher SES. Conclusions: SES may be linked to psychopathology via blunted sensitivity to acceptance . These findings suggest that prevention and intervention efforts targeting sensitivity to social feedback may reduce risk for psychopathology particularly in low SES youth.
...
Low socioeconomic status (SES) in adolescence is a known correlate [@mclaughlinSocioeconomicStatusAdolescent2012; @peverillSocioeconomicStatusChild2021] ) and longitudinal predictor of psychopathology and general health problems into adulthood [@ferraroChildhoodDisadvantageHealth2016; @santiagoSocioeconomicStatusNeighborhood2011]. Yet, not all adolescents who experience low SES will develop mental illness. Thus, it is imperative to identify factors which could help explain why some low SES youth develop mental illness, while others do not. This study examines whether one potential factor—neural processing of social feedback from peers - interacts with SES to influence individual affective dynamics.

Fulfilling relationships in adolescence predict long term mental health and functioning [@landstedtHowWellParental2015; @adedejiPeerRelationshipsDepressive2022], and the salience of peer relationships increases in adolescence [@nelsonSocialReorientationBrain2016; @venticinqueExpandingUnderstandingAdolescent2024]. However, compared to higher SES youth, lower SES youth experience greater discrimination and stigma from others [@cozzarelliAttitudesPoorAttributions2001; @mouraStigmatizationPovertyBasis2019]; and this can lead to difficulty being trusting and intimate with others [@rokachChapter11Implications2023; @nesdalePeerRejectionChildhood2013]. As such, low SES adolescents may develop a bias to feedback from others [@simonsRelationalSchemasHostile2012; @simonsLearningBeBad2011], including anhedonic responses to positive feedback (i.e., acceptance). Over time, these biases can lead to social withdrawal and mental health problems [@ikeSocialWithdrawalInitially2020].
...
Omnibus MLMs showed that Peer value interacted with Feedback type to predict ERP mean amplitude for the RewP (`r apa_print(rewp_fv_mlm, est_name="\\beta")$full_result$FeedbackRej_VotingRej`) and P300 (`r apa_print(p300_fv_mlm, est_name="\\beta")$full_result$FeedbackRej_VotingRej`). Follow-up analyses showed that the RewP (`r paste0(d_rewp_lv, ", ", apa_printt(ttest_rewp_lv)$full_result)`) and P300 (`r paste0(d_p3_lv, ", ", apa_printt(ttest_p3_lv)$full_result)`) differentiated between acceptance and rejection for low-value peers. Only the RewP differentiated between acceptance and rejection for high-value peers (`r paste0(d_rewp_hv, ", ", apa_printt(ttest_rewp_hv)$full_result)`), but not the P300 (`r paste0(d_p3_lv, ", ", apa_printt(ttest_p3_lv)$full_result)`).
...
ses_lm <- function(data, y) {
  ...
  model_formula <- formula(paste0(quo_name(y), "~ ADI_NATRANK_z + site + Age_c + demo_child_sex + StressCT + StressTH + IDAS_depression_z + IDAS_anxiety"))
...
We found support for aim 1: SES was significantly related to brain responses to different kinds of peer feedback (see Figure 2). Specifically, lower SES was significantly related to a more blunted RewP~resid~ and P3~resid~ to acceptance from high-value peers (i.e., peers the participant was interested in). These findings remained significant over-and-above site, demographic factors (e.g., age, sex assigned at birth), lifetime stressors (total stressful life events, severity of stressful life events), and self-reported psychopathology (self-reported depression, self-reported anxiety). SES was not related to RewP~resid~ nor P300~resid~ to rejection (`r apa_printlm(rewp_araa_model)$full_result$ADI_NATRANK_z`; `r apa_printlm(p300_araa_model)$full_result$ADI_NATRANK_z`, respectively). Additionally, SES was not related to RewP~resid~ or P3~resid~ to acceptance or rejection from *low-value* peers (i.e., peers the participant was not interested in, $p \ge$ `r round(min(rewp_low_value_ses_pvalues,p300_low_value_ses_pvalues),2)`). Results are consistent when also covarying for self-reported deception (see Supplement).

***SES does not moderate association between RewP~resid~/P3~resid~ and depression or anxiety symptoms***

```{r}
model_150275_dep <- lm(IDAS_depression_z ~ AA_AR_z + Age_z + demo_child_sex + StressCT_z + StressTH_z + site, data = df_150275_cz)

model_275425_dep <- lm(IDAS_depression_z ~ AA_AR_z + Age_z + demo_child_sex + StressCT_z + StressTH_z + site, data = df_275425_pz)

model_150275_anx <- lm(IDAS_anxiety_z ~ AA_AR_z + Age_z + demo_child_sex + StressCT_z + StressTH_z + site, data = df_150275_cz)

model_275425_anx <- lm(IDAS_anxiety_z ~ AA_AR_z + Age_z + demo_child_sex + StressCT_z + StressTH_z + site, data = df_275425_pz)

model_150275_dep_ses <- lm(IDAS_depression_z ~ ADI_NATRANK_z*AA_AR_z + Age_z + demo_child_sex + StressCT_z + StressTH_z + site, data = df_150275_cz)

model_275425_dep_ses <- lm(IDAS_depression_z ~ ADI_NATRANK_z*AA_AR_z + Age_z + demo_child_sex + StressCT_z + StressTH_z + site, data = df_275425_pz)

model_150275_anx_ses <- lm(IDAS_anxiety_z ~ ADI_NATRANK_z*AA_AR_z + Age_z + demo_child_sex + StressCT_z + StressTH_z + site, data = df_150275_cz)

model_275425_anx_ses <- lm(IDAS_anxiety_z ~ ADI_NATRANK_z*AA_AR_z + Age_z + demo_child_sex + StressCT_z + StressTH_z + site, data = df_275425_pz)
```

***SES moderates association between RewP~resid~/P3~resid~ and affect***

```{r}
model_150275_ema_na <- glmmTMB(na_cubert ~ ADI_NATRANK_z*AA_AR_z + Age_z + demo_child_sex + IDAS_depression_z + IDAS_anxiety_z + StressCT_z + StressTH_z + site + (1 | ID),
                zi = ~ .,
  data = df_150275_cz_ema,
  family=gaussian, REML=TRUE)

model_275425_ema_na <- glmmTMB(na_cubert ~ ADI_NATRANK_z*AA_AR_z + Age_c + demo_child_sex + IDAS_depression_z + IDAS_anxiety_z + StressCT_z + StressTH_z + site + (1 | ID),
                                   zi = ~ .,
...
m_p300_acc <- glmmTMB(na_cubert ~ ADI_NATRANK_z*AA_AR_z + Age_c + demo_child_sex + site + (1 | ID),
  ...
  data = df_275425_pz_ema,
  family=gaussian, REML=TRUE)
#IDAS_depression_z + IDAS_anxiety_z + StressCT_z + StressTH_z + 
...
summary(lm(AA_AR_z ~ IDAS_depression_z, df_150275_cz_groups))
summary(lm(AR_AA_z ~ IDAS_depression_z, df_150275_cz_groups))
...
summary(lm(AA_AR_z ~ IDAS_depression_z, filter(df_150275_cz_groups, group!="HC")))
summary(lm(AR_AA_z ~ IDAS_depression_z, filter(df_150275_cz_groups, group!="HC")))

summary(lm(AA_AR_z ~ IDAS_depression_z, filter(df_275425_pz_groups, group!="HC")))
summary(lm(AR_AA_z ~ IDAS_depression_z, filter(df_275425_pz_groups, group!="HC")))
...
summary(lm(AR_AA_z ~ scale(IDAS_depression_z, center=TRUE, scale=TRUE), df_275425_pz_groups))
summary(lm(AR_AA_z ~ scale(IDAS_depression_z, center=TRUE, scale=TRUE), filter(df_275425_pz_groups, group=="remMDD")))
summary(lm(AR_AA_z ~ scale(IDAS_depression_z, center=TRUE, scale=TRUE) + Age + demo_child_sex, filter(df_275425_pz_groups, group!="HC")))
summary(lm(AR_AA_z ~ scale(IDAS_depression_z, center=TRUE, scale=TRUE) + Age + demo_child_sex, filter(df_275425_pz_groups, group=="remMDD")))

summary(lm(AA_AR_z ~ group + scale(IDAS_depression_z, center=TRUE, scale=TRUE) + Age + demo_child_sex, filter(df_150275_cz_groups, group!="MDD")))
summary(lm(AR_AA_z ~ group + scale(IDAS_depression_z, center=TRUE, scale=TRUE) + Age + demo_child_sex, filter(df_150275_cz_groups, group!="MDD")))
summary(lm(AA_AR_z ~ group + scale(IDAS_depression_z, center=TRUE, scale=TRUE) + Age + demo_child_sex, filter(df_275425_pz_groups, group!="MDD")))
summary(lm(AR_AA_z ~ group + scale(IDAS_depression_z, center=TRUE, scale=TRUE) + Age + demo_child_sex, filter(df_275425_pz_groups, group!="MDD")))
...
sum(complete.cases(filter(df_275425_pz_groups, group=="remMDD")$IDAS_depression_z))

summary(lm(AR_AA_z ~ scale(IDAS_depression_z, center=TRUE, scale=TRUE) + Age + demo_child_sex, filter(df_275425_pz_groups, group!="HC")))
df_p3_no_hc <- filter(df_275425_pz_groups, group!="HC")
df_rewp_no_hc <- filter(df_150275_cz_groups, group!="HC")

# Task effects
t.test(df_p3_no_hc$Acc_Acc, df_p3_no_hc$Acc_Rej, paired=TRUE)
t.test(df_rewp_no_hc$Acc_Acc, df_rewp_no_hc$Acc_Rej, paired=TRUE)
t.test(df_150275_cz_groups$Acc_Acc, df_150275_cz_groups$Acc_Rej, paired=TRUE)

model1 <- lm(AR_AA_z ~ scale(IDAS_depression_z, center=TRUE, scale=TRUE) + Age + demo_child_sex, df_no_hc)
df_p3_no_hc$fitted <- fitted(model1)
# Load ggplot2
library(ggplot2)

# Assuming you have a data frame called 'df' with columns 'IDAS_depression_z' and 'AR_AA_z'
plot1 <- ggplot(df_p3_no_hc, aes(x = AR_AA_z, y = fitted)) +
