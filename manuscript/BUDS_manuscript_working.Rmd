...
[^2]: This somewhat surprising/counterintuitive finding is discussed further in the Discussion.
...
######## ACC > REJ ######## 
#m_p300_acc <- glmmTMB(na_cubert ~ ADI_NATRANK_z*AA_AR_z + Age_c + demo_child_sex + pa + (1 | ID) + site,
#        zi = ~ .,
#  data = df_275425_cz_ema,
#  family=gaussian, REML=TRUE)
#
#m_p300_acc_ss <- emtrends(m_p300_acc, pairwise ~ADI_NATRANK_z, var="AA_AR_z",at=SES_list, adjust="none")|> test()
#m_p300_acc_cov_ss <- emtrends(model_275425_ema_na_cov, pairwise ~ADI_NATRANK_c, var="AA_AR_c",at=SES_list, adjust="none")|> test()
...
Finally, we examined whether RewP~resid~ or P300~resid~ differed by hispanic ethnicity or participant reported race, given that minority youth are at greater likelihood of being discriminated against. We did not find significant differences for hispanic ethnicity (``` r apa_print(model_150275_his``, symbol="beta"``)$full_result$demo_child_hispanicYes ```, ``` r apa_print(model_150275_hisp``, symbol="beta"``)$full_result$demo_child_hispanicYes ```, respectively), nor minority race (``` r apa_print(model_150275_race``, symbol="beta"``)$full_result$demo_child_minority ```, ``` r apa_print(model_275425_race``, symbol="beta"``)$full_result$demo_child_minority ```, respectively).
...
```{r, eval=FALSE}
df_150275_cz_strain <- df_150275_cz_strain %>%
  mutate(site = case_when(ID <9000 ~ "Columbia",
                          ID >8999 ~ "Northwestern"))
df_275425_cz_strain <- df_275425_cz_strain %>%
  mutate(site = case_when(ID <9000 ~ "Columbia",
                          ID >8999 ~ "Northwestern"))
demo_table <- FullTable1(df_150275_cz_strain, strata="site",
           vars=c("Age","ADI_NATRANK","demo_child_sex","demo_child_hispanic","demo_child_race",
                  "IDAS_depression","IDAS_wellbeing"),
           factor_vars=c("demo_child_sex","demo_child_hispanic","demo_child_race"),
           var_names=c("Age","SES","Sex","Hispanic ethnicity","Race",
                       "IDAS depression","IDAS positive affect"),
           round_n=2)

summary(lmer(na ~ site + (1 | ID), df_275425_cz_ema))
df_275425_cz_ema %>%
  dplyr::group_by(site) %>%
  dplyr::summarise(mean = mean(na, na.rm=TRUE),
            sd = sd(na, na.rm=TRUE))

summary(lmer(pa ~ site + (1 | ID), df_275425_cz_ema))
df_275425_cz_ema %>%
  dplyr::group_by(site) %>%
  dplyr::summarise(mean = mean(pa, na.rm=TRUE),
            sd = sd(pa, na.rm=TRUE))

write.csv(demo_table$table, "../tables/demo_table_SOBP.csv")

#####
p3_aaar_model <- lm(formula = AA_AR_z ~ ADI_NATRANK_z + site + Age + demo_child_sex + IDAS_depression, data = df_275425_cz_strain)
summary(lm(formula = AR_AA_z ~ ADI_NATRANK_z + site + Age + demo_child_sex + IDAS_depression, data = df_275425_cz_strain))
scatterplot_275425_adi <- ggplot(df_275425_cz, aes(x=ADI_NATRANK, y=AA_AR_z)) +
  geom_point(colour="black") +
  stat_smooth(method="lm", se=TRUE) +
  theme_apa() +
  labs(x="SES", y="P300 (µV) residual \nto acceptance") +
                                       coord_cartesian(xlim=c(0,100), ylim=c(-3,4)) +
                                       theme(text = element_text(size=18)) + 
                                       annotate("text", x=40, y=3.5, 
                                                label = TeX(apa_print(p3_aaar_model)$full_result$ADI_NATRANK_z), 
                                                color = "black", size=5)

ggsave("../figures/scatterplot_275425_adi.png",scatterplot_275425_adi, width=9, height=4.5)

model_275425_ema_na_AR <- glmmTMB(na_cubert ~ ADI_NATRANK_z*AR_AA_z + Age_c + demo_child_sex + pa + (1 | ID) + (1 | site),
        zi = ~ .,
  data = df_275425_cz_ema,
  family=gaussian, REML=TRUE)
...
plot_275425_adi_na_rej <- ggpredict(model_275425_ema_na_AR, terms = c("AR_AA_z","ADI_NATRANK_z")) %>%
  plot() + 
  theme_apa() +
  labs(x="P300 (µV) residual to rejection", y="Negative affect (cubed root)", color="SES", linetype="SES", title="") +
  scale_color_manual(values=c('orange','white','darkblue'), labels=c('-SD', '+SD')) + 
  scale_fill_manual(values=c('orange','white','darkblue')) +
  theme(legend.position = "none", text = element_text(size=18))

ggsave("../figures/plot_275425_adi_na_rej.png",plot_275425_adi_na_rej, width=9, height=4.5)

p3_aaar_model_disc <- lm(AA_AR_z ~ addi_total_z + Age_c + demo_child_sex + site + IDAS_depression, df_275425_cz_strain)

scatterplot_275425_disc <- ggplot(df_275425_cz_strain, aes(x=addi_total, y=AA_AR_z)) +
  geom_point(colour="black") +
  stat_smooth(method="lm", se=TRUE) +
  theme_apa() +
  labs(x="Self-reported discrimination", y="P300 (µV) residual \nto acceptance") +
  theme(text=element_text(size=16)) +
  coord_cartesian(xlim = c(-0.5, 31), ylim=c(-3,4)) +
  annotate("text", x=15, y=3.5, label = TeX(apa_print(p3_aaar_model_disc)$full_result$addi_total_z), color = "black", size=4)

ggsave("../figures/scatterplot_275425_disc.png",scatterplot_275425_disc, width=7, height=4.5)

summary(glmmTMB(na_cubert ~ AR_AA_z + (1 | ID) + (1 | site),
        zi = ~ .,
  data = df_275425_cz_ema,
  family=gaussian, REML=TRUE))

summary(glmmTMB(na_cubert ~ addi_total_z*AR_AA_z + Age_c + demo_child_sex + (1 | ID) + (1 | site),
        zi = ~ .,
  data = df_275425_cz_ema,
  family=gaussian, REML=TRUE))
...
# REWP
rewp_aaar_model <- lm(formula = AA_AR_z ~ ADI_NATRANK_z + site + Age + demo_child_sex + IDAS_depression, data = df_150275_cz_strain)
scatterplot_150275_adi <- ggplot(df_150275_cz, aes(x=ADI_NATRANK, y=AA_AR_z)) +
  geom_point(colour="black") +
  stat_smooth(method="lm", se=TRUE) +
  theme_apa() +
  labs(x="SES", y="RewP (µV) residual \nto acceptance") +
                                       coord_cartesian(xlim=c(0,100), ylim=c(-3,4)) +
                                       theme(text = element_text(size=16)) + 
                                       annotate("text", x=40, y=3.5, 
                                                label = TeX(apa_print(rewp_aaar_model)$full_result$ADI_NATRANK_z), 
                                                color = "black", size=6)
ggsave("../figures/scatterplot_150275_adi.png",scatterplot_150275_adi, width=9, height=4.5)

model_150275_ema_na <- glmmTMB(na_cubert ~ ADI_NATRANK_z*AA_AR_z + Age_c + demo_child_sex + pa + (1 | ID) + site,
        zi = ~ .,
  data = df_150275_cz_ema,
  family=gaussian, REML=TRUE)
glmmTMB:::Anova.glmmTMB(model_150275_ema_na)
...
######## ACC > REJ ######## 
m_rewp_acc <- glmmTMB(na_cubert ~ ADI_NATRANK_c*AA_AR_c + Age_c + demo_child_sex + site + (1 | ID),
        zi = ~ .,
  data = df_150275_cz_ema,
  family=gaussian, REML=TRUE)
...
plot_150275_adi_na <- ggpredict(model_150275_ema_na, terms = c("AA_AR_z","ADI_NATRANK_z")) %>%
  plot() + 
  theme_apa() +
  labs(x="RewP (µV) residual to acceptance", y="Negative affect (cubed root)", color="SES", linetype="SES", title="") +
  scale_color_manual(values=c('orange','white','darkblue'), labels=c('-SD', '+SD')) + 
  scale_fill_manual(values=c('orange','white','darkblue')) +
  theme(legend.position = "none", text = element_text(size=18))

ggsave("../figures/plot_150275_adi_na.png",plot_150275_adi_na, width=8, height=4)

rewp_aaar_model_disc <- lm(formula = AA_AR_z ~ addi_total_z + site + Age_c + demo_child_sex + IDAS_depression, data = df_150275_cz_strain)

scatterplot_150275_disc <- ggplot(df_150275_cz_strain, aes(x=addi_total, y=AA_AR_z)) +
  geom_point(colour="black") +
  stat_smooth(method="lm", se=TRUE) +
  theme_apa() +
  labs(x="Self-reported discrimination", y="RewP (µV) residual \nto acceptance") +
  theme(text=element_text(size=16)) +
  coord_cartesian(xlim = c(-0.5, 31), ylim=c(-3,3)) +
  annotate("text", x=15, y=3, label = TeX(apa_print(rewp_aaar_model_disc)$full_result$addi_total_z), color = "black", size=5)

ggsave("../figures/scatterplot_150275_disc.png",scatterplot_150275_disc, width=8, height=4)
```

# Figures

Figure 1. Grand average ERP for acceptance and rejection feedback from high-value peers. Colored sections of the graph represent time windows where ERP components were extracted at Cz electrode: N1 = purple, RewP = orange, P300 = blue.

Figure 2. Mean ratings across all participants by Feedback and Value.

Figure 3. Relationship between SES and brain responses to acceptance (relative to rejection) for both the RewP and P300 ERP components.

Figure 4. Interactions for which SES significantly moderated associated between brain responses to acceptance/rejection and negative affect. A: no simple slopes significant. B: Simple slope significant for SES -1 standard deviation below the mean.
