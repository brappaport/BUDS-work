...
The RewP and P300 demonstrated acceptable levels of internal consistency across peer-value (high, low) and feedback conditions (acceptance, rejection), with split-half reliability (*r*) \> `r min(spearman_brown[2:3,3:6])`, with particularly high reliability for the high-value peers conditions (acceptance and rejection *r* = `r spearman_brown[2:3,3]` and `r spearman_brown[2:3,4]`; see Supplement). The N100 did not achieve acceptable internal consistency (*r*=`r min(spearman_brown[1,3:6])`-`r max(spearman_brown[1,3:6])`). Split-half reliability was considerably lower for the residualized scores, but remained comparable to other studies (RewP~resid~ *r*\>`r min(spearman_brown[2,7:8])`, P300~resid~ *r*\>`r min(spearman_brown[3,7:8])`) [@rappaportBehavioralPsychiatricCorrelates2024a; @ethridgePsychometricPropertiesNeural2018; @levinsonReliabilityElectrocorticalResponse2017]. See Supplement for full results.
...
```{r}
# RewP
model_150275_addi <- lm(AA_AR_z ~ addi_total_z + Age_c + demo_child_sex + site, df_150275_cz_strain)
model_150275_addi_ar <- lm(AR_AA_z ~ addi_total_z + Age_c + demo_child_sex + site, df_150275_cz_strain)
# P300
model_275425_addi <- lm(AA_AR_z ~ addi_total_z + Age_c + demo_child_sex + site, data = df_275425_pz_strain)
model_275425_addi_ar <- lm(AR_AA_z ~ addi_total_z + Age_c + demo_child_sex + site, data = df_275425_pz_strain)
```

Greater self-reported discrimination distress was related to a more blunted RewP~resid~ (`r apa_print(model_150275_addi)$full_result$addi_total_z`) and P300~resid~ (`r apa_print(model_275425_addi)$full_result$addi_total_z`) to acceptance and more heightened response RewP~resid~ (`r apa_print(model_150275_addi_ar)$full_result$addi_total_z`) and P300~resid~ to rejection (`r apa_print(model_275425_addi_ar)$full_result$addi_total_z`). However, these results are considered preliminary since only `r sum(df_50150_fz$addi_total>0, na.rm=TRUE)` (`r round(sum(df_50150_fz$addi_total>0, na.rm=TRUE)/sum(complete.cases(df_50150_fz$addi_total))*100,2)`%) participants reported having experienced *any* form of discrimination.
...
The present study also explored whether a key aspect of SES—discrimination—relates to neural response to social feedback. The findings support and add to theories that discrimination leads to heightened threat-sensitive responses to peer rejection [@faniAssociationRacialDiscrimination2021; @chanStatusBasedRejectionSensitivity2008]. We show that in addition to heightened sensitivity to rejection, discrimination may also be tied to blunted hedonic responses to peer acceptance. Future research could further test whether discrimination experiences moderate or are mediated by associations between neural sensitivity to acceptance/rejection and risk for psychopathology, as recent research suggests [@dyarMediatingRolesRejection2018; @feinsteinRelationshipExperiencesDiscrimination2012].
