...
During post-task debriefing, `r table(df_BUDS_rating_deception$SP_debriefing_deceived)[1]` participants responded 'yes' and `r table(df_BUDS_rating_deception$SP_debriefing_deceived)[2]` participants responded 'no' to the question "While you were doing the chatroom, did you believe that you would really be chatting with one of these people on the computer after your EEG?" (N= `r sum(is.na(df_BUDS_rating_deception$SP_debriefing_deceived))` were missing debriefing data). Given the potential for deception to affect results, we present additional analyses covarying for this in the Supplement.
...
```{r}
p300_aaar_model <- lm(formula = AA_AR_z ~ ADI_NATRANK_z + site + Age + demo_child_sex + StressCT + StressTH + IDAS_depression + IDAS_anxiety, data = df_275425_cz_strain)
p300_araa_model <- lm(formula = AR_AA_z ~ ADI_NATRANK_z + site + Age + demo_child_sex + StressCT + StressTH + IDAS_depression + IDAS_anxiety, data = df_275425_cz_strain)

p300_rarr_model <- lm(formula = RA_RR ~ ADI_NATRANK_z + site + Age + demo_child_sex + StressCT + StressTH + IDAS_depression + IDAS_anxiety, data = df_275425_cz_strain)
p300_rrra_model <- lm(formula = RR_RA ~ ADI_NATRANK_z + site + Age + demo_child_sex + StressCT + StressTH + IDAS_depression + IDAS_anxiety, data = df_275425_cz_strain)
...
We found support for aim 1: SES was significantly related to brain responses to different kinds of peer feedback (see Figure 2). Specifically, lower SES was significantly related to a more blunted RewP~resid~ (`r apa_printlm(rewp_aaar_model)$full_result$ADI_NATRANK_z`) and P300~resid~ to acceptance from high-value peers (i.e., that the participant was interested in, `r apa_printlm(p300_aaar_model)$full_result$ADI_NATRANK_z`). This is over-and-above site, demographic factors (e.g., age, sex assigned at birth), lifetime stressors (total stressful life events, severity of stressful life events), and self-reported psychopathology (self-reported depression, self-reported anxiety). SES was not related to RewP~resid~ and P300~resid~ to rejection (`r apa_printlm(rewp_araa_model)$full_result$ADI_NATRANKz_z`; `r apa_printlm(p300_araa_model)$full_result$ADI_NATRANK_z`, respectively). Additionally, SES was not related to RewP~resid~ or P300~resid~ to acceptance or rejection from *low-value* peers (i.e., that the participant was not interested in, $p \ge$ `r round(min(rewp_low_value_ses_pvalues,p300_low_value_ses_pvalues),2)`). Results are consistent when also covarying for self-reported deception (see Supplement).
...
We also found support for aim 2: SES significantly moderated the relationship between RewP~resid~ to acceptance and negative affect ($\hat{\beta}$=`r rewp_na_stat$estimate`, 95% CI=[`r paste0(rewp_na_stat$lower,", ", rewp_na_stat$upper)`], $p$=`r rewp_na_stat$p.value`), as well as P300~resid~ to rejection and negative affect ($\hat{\beta}$=`r p300_na_ar_stat$estimate`, 95% CI=[`r paste0(p300_na_ar_stat$lower,", ",p300_na_ar_stat$upper)`], $p$=`r p300_na_ar_stat$p.value`, see Figure 3). The SES x P300~resid~ rejection interaction remained significant even when accounting for positive affect as a fixed effect covariate ($\hat{\beta}$=`r p300_na_ar_pa_stat$estimate`, 95% CI=[`r paste0(p300_na_ar_pa_stat$lower,", ", p300_na_ar_pa_stat$upper)`], $p$=`r p300_na_ar_pa_stat$p.value`). These effects were consistent when covarying for deception (see Supplement).

SES did *not* moderate the relationship between RewP~resid~ to rejection and negative affect ($\hat{\beta}$=`r rewp_na_ar_stat$estimate`, 95% CI=[`r paste0(rewp_na_ar_stat$lower,", ", rewp_na_ar_stat$upper)`], $p$=`r rewp_na_ar_stat$p.value`) nor P300~resid~ to acceptance and negative affect ($\hat{\beta}$=`r p300_na_stat$estimate`, 95% CI=[`r paste0(p300_na_stat$lower,", ", p300_na_stat$upper)`], $p$=`r p300_na_stat$p.value`).

SES also did not moderate the relationship between RewP~resid~ to acceptance and positive affect ($\hat{\beta}$=`r rewp_pa_stat$estimate`, 95% CI=[`r paste0(rewp_pa_stat$lower,", ", rewp_pa_stat$upper)`], $p$=`r rewp_pa_stat$p.value`) , nor between P300~resid~ to rejection and positive affect ($\hat{\beta}$=`r p300_pa_stat$estimate`, 95% CI=[`r paste0(p300_pa_stat$lower,", ", p300_pa_stat$upper)`], $p$=`r p300_pa_stat$p.value`).
...
Finally, we examined whether RewP~resid~ or P300~resid~ differed by Hispanic ethnicity or participant reported race, given that racial/ethnic minority youth are at greater likelihood of being discriminated against. We did not find significant differences for Hispanic ethnicity (`r apa_printlm(model_150275_hisp)$full_result$demo_child_hispanicYes`, `r apa_printlm(model_150275_hisp)$full_result$demo_child_hispanicYes`, respectively), nor minority race (`r apa_printlm(model_150275_race)$full_result$demo_child_minority`, `r apa_printlm(model_275425_race)$full_result$demo_child_minority`, respectively).
