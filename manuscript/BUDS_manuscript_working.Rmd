...
load(file=here("../data/df_50150.RData"))
load(file=here("../data/df_150275.RData"))
load(file=here("../data/df_275425.RData"))
...
# Results NEED TO UPDATE FROM HERE ON
...
In dependability analyses, the RewP and P300 achieved acceptable dependability ($\ge$ 0.70) with as few as 12-13 and 13-15 trials, respectively, depending on the peer-value and feedback condition. When using all available trials, up to 25 trials per condition, all ERPs achieved good dependability ($\ge$ 0.80; see Supplement). The N100 did not achieve acceptable dependability with any number of trials (as many as 25 per condition).
...
```{r, eval=FALSE}
length(unique(filter(df_150275_cz_ema, complete.cases(na_cubert), 
                     complete.cases(AA_AR_z), 
                     complete.cases(ADI_NATRANK_z), 
                     complete.cases(demo_child_sex),
                     complete.cases(Age_c))$ID)) # SHOULD BE 133

length(unique(filter(df_275425_pz_ema, complete.cases(na_cubert), 
                     complete.cases(AA_AR_z), 
                     complete.cases(ADI_NATRANK_z), 
                     complete.cases(demo_child_sex),
                     complete.cases(Age_c))$ID)) # SHOULD BE 125
```
