...
#### ***SES moderating association between in-task emotional response rating and EMA measures of affect.*** 
...
SES did *not* moderate the relationship between RewP~resid~ to rejection ($\hat{\beta}$ = `r rewp_na_ar_stat$estimate`, 95% CI = [`r paste0(rewp_na_ar_stat$lower,", ", rewp_na_ar_stat$upper)`], $p$ = `r rewp_na_ar_stat$p.value`) nor P300~resid~ to acceptance ($\hat{\beta}$ = `r p300_na_stat$estimate`, 95% CI = [`r paste0(p300_na_stat$lower,", ", p300_na_stat$upper)`], $p$ = `r p300_na_stat$p.value`). These effects were consistent when covarying for deception (see Supplement).

SES also did not moderate the relationship between RewP~resid~ to acceptance () and positive affect, nor between P300~resid~ to rejection and positive affect ().
