...
```
### Interpretation of residualized scores
```{r, eval=FALSE}
ggplot(df_150275_cz,aes(x=AA_AR, y=AA_AR_diff)) +
  geom_point() +
  stat_smooth(method="lm") +
  labs(x="residualized RewP", y="subtraction based RewP")
summary(lm(AA_AR_diff ~ AA_AR, df_150275_cz))

ggplot(df_275425_cz,aes(x=AA_AR, y=AA_AR_diff)) +
  geom_point() +
  stat_smooth(method="lm") +
  labs(x="residualized P3", y="subtraction based P3")
summary(lm(AA_AR_diff ~ AA_AR, df_275425_cz))
```
...
lmer_interaction_table <- function(data) {
  new_model <- data.frame(component = deparse(substitute(data)),
               beta = broom.mixed::tidy(lmer(formula = ERP ~ Feedback*Voting + site + (1 | ID), REML=TRUE, data = data))[5,4], 
               p = broom.mixed::tidy(lmer(formula = ERP ~ Feedback*Voting + site + (1 | ID), REML=TRUE, data = data))[5,8])
  return(new_model)
}
...
summary(lmer(formula = ERP ~ Feedback*ADI_NATRANK_z + Age + Sex + StressCT + StressTH + IDAS_wellbeing + site + (1 | ID), data = filter(df_150275_cz_long_strain, Voting=="Like")))
...
### P3
```{r}
summary(lmer(formula = ERP ~ Feedback*ADI_NATRANK_c + Age + Sex + StressCT + StressTH + IDAS_wellbeing + (1 | ID) + (1 | site), data = filter(df_275425_cz_long_strain, Voting=="Like")))
...
#### Simple slopes
```{r}
mean_ADI <- mean(df_275425_cz_ema$ADI_NATRANK_c, na.rm=TRUE)
sd_ADI <- sd(df_275425_cz_ema$ADI_NATRANK_c, na.rm=TRUE)
...
interact_plot(glmmTMB(na_cubert ~ ADI_NATRANK_c*AA_AR_c + (1 | ID) + (1 | site),
                    zi = ~ ADI_NATRANK_c*AA_AR_c + (1 | ID) + (1 | site),
    data = df_275425_cz_ema,
    family=gaussian, REML=TRUE, na.action=na.exclude),
    data = df_275425_cz_ema,
    pred = ADI_NATRANK_c, modx = AA_AR_c, plot.points = TRUE,
              modx.values = c(mean_p3-sd_p3,mean_p3+sd_p3),point.alpha = 0.2, colors="CUD Bright")

plot(ggeffects::johnson_neyman(ggpredict(m_acc, 
                                         terms = c("ADI_NATRANK","AA_AR")),
                               vcov = sandwich::vcovHC))
```
ADI as the moderator
```{r}
ADIa <- mean(df_275425_cz_ema$ADI_NATRANK, na.rm=T) + sd(df_275425_cz_ema$ADI_NATRANK, na.rm=T)
ADIb <- mean(df_275425_cz_ema$ADI_NATRANK, na.rm=T) - sd(df_275425_cz_ema$ADI_NATRANK, na.rm=T)
ADI_list <- list(ADI_NATRANK=round(c(ADIa,ADIb),2)) 

m_acc <- glmmTMB(na_cubert ~ ADI_NATRANK*AA_AR + (1 | ID) + (1 | site),
                    zi = ~ ADI_NATRANK*AA_AR + (1 | ID) + (1 | site),
    data = df_275425_cz_ema,
    family=gaussian, REML=FALSE, na.action=na.exclude)

adi_ss <- emtrends(m_acc, pairwise ~ADI_NATRANK, var="AA_AR",at=ADI_list, adjust="none")|> test()

plot(ggeffects::johnson_neyman(ggpredict(m_acc, 
                                         terms = c("AA_AR","ADI_NATRANK")),
                               vcov = sandwich::vcovHC))


interact_plot(m_acc,
    data = df_275425_cz_ema,
    pred = AA_AR, modx = ADI_NATRANK, plot.points = TRUE,
              modx.values = c(mean_ADI-sd_ADI,mean_ADI+sd_ADI),
...
P300 as the moderator
```{r}
P3a <- mean(df_275425_cz_ema$AR_AA) + sd(df_275425_cz_ema$AR_AA)
P3b <- mean(df_275425_cz_ema$AR_AA) - sd(df_275425_cz_ema$AR_AA)
P3_list <- list(AR_AA=round(c(P3a,P3b),2)) 

model_275425_ema_na_AR <- glmmTMB(na_cubert ~ ADI_NATRANK_c*AR_AA + (1 | ID) + (1 | site),
                    zi = ~ ADI_NATRANK_c*AR_AA + (1 | ID) + (1 | site),
    data = df_275425_cz_ema,
    family=gaussian, REML=TRUE, na.action=na.exclude)

emtrends(model_275425_ema_na_AR, pairwise ~AR_AA, var="ADI_NATRANK_c",at=P3_list, adjust="none")|> test()
...
ADI as the moderator
```{r}
emtrends(glmmTMB(na_cubert ~ ADI_NATRANK*AR_AA + (1 | ID) + (1 | site),
                    zi = ~ ADI_NATRANK*AR_AA + (1 | ID) + (1 | site),
    data = df_275425_cz_ema,
    family=gaussian, REML=TRUE, na.action=na.exclude),
pairwise ~ADI_NATRANK, var="AR_AA",at=ADI_list, adjust="none")|> test()

interact_plot(glmmTMB(na_cubert ~ ADI_NATRANK*AR_AA + (1 | ID) + (1 | site),
                    zi = ~ ADI_NATRANK*AR_AA + (1 | ID),
    data = df_275425_cz_ema,
    family=gaussian, REML=TRUE, na.action=na.exclude),
    data = df_275425_cz_ema,
    pred = AR_AA, modx = ADI_NATRANK, plot.points = TRUE,
              modx.values = c(round(mean_ADI-sd_ADI,0),round(mean_ADI+sd_ADI,0)),
    point.alpha = 0.2, colors="CUD Bright")

m_rej <- glmmTMB(na_cubert ~ ADI_NATRANK*AR_AA + (1 | ID) + (1 | site),
                    zi = ~ ADI_NATRANK*AR_AA + site + (1 | ID),
    data = df_275425_cz_ema,
    family=gaussian, REML=FALSE, na.action=na.exclude)

plot(ggeffects::johnson_neyman(ggpredict(m_rej,
                                         terms = c("AR_AA","ADI_NATRANK")),
                               vcov = sandwich::vcovHC))
```
When examining response to acceptance, those with an ADI greater than 32nd percentile show a negative relationship between P300 and negative affect, such that those with a smaller response to rejection experience less negative affect.


When examining response to rejection, those with an ADI greater than 24th percentile show a positive relationship between P300 and negative affect, such that those with a greater response to rejection experience greater negative affect.
...
### Covaring for age, sex, hispanic ethnicity, stress, IDAS
```{r}
model_150275_ema_na_cov <- glmmTMB(na_cubert ~ ADI_NATRANK_c*AA_AR_c + Age + Sex + demo_child_hispanic + StressCT + StressTH + IDAS_depression + IDAS_wellbeing + IDAS_panic + IDAS_sa + (1 | ID) + (1 | site),
                    zi = ~ ADI_NATRANK_c*AA_AR_c + (1 | ID),
    data = df_150275_cz_ema,
    family=gaussian, REML=TRUE)
...
model_275425_ema_na_cov <- glmmTMB(na_cubert ~ ADI_NATRANK_c*AA_AR_c + 
                                     Age + Sex + StressCT + StressTH + IDAS_wellbeing + 
                                     (1 | ID) + (1 | site),
