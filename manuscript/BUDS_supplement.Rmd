...
```{r eval=FALSE, remove subjects with too few trials}
bad_150275 <- c(1025,
1055,
1084,
1090,
...
### N1
```{r, eval=T}
# models_50150_fz_table <- lmer_adi_interaction_table(df_50150_fz_long)
# models_50150_fz_table$component <- "50150_fz"

model_50150_fz <- lmer(formula = ERP ~ Feedback*ADI_NATRANK_z*Voting + (1 | ID) + (1 | site), data = df_50150_fz_long)
model_50150_fz_high <- lmer(formula = ERP ~ Feedback*ADI_NATRANK_z + (1 | ID) + (1 | site), data = filter(df_50150_fz_long,Voting=="Like"))
model_50150_fz_low <- lmer(formula = ERP ~ Feedback*ADI_NATRANK_z + (1 | ID) + (1 | site), data = filter(df_50150_fz_long,Voting=="Dislike"))
...
plot_50150_adi_t <- data.frame(
  label = c(plot_stats(model_50150_fz_high, "Feedback1:ADI_NATRANK_z"),
            plot_stats(model_50150_fz_low, "Feedback1:ADI_NATRANK_z")), 
  Value  = c("High", "Low"),
...
df_275425_pz_rating2 <- df_275425_pz_rating %>%
  select(ID, SP_debriefing_deceived) %>%
  full_join(df_BUDS_rating_it_pt_demo, by="ID") %>%
  select(ID, Age, StressCT, StressTH, SP_debriefing_deceived) %>%
...
p3_aaar_model <- ses_lm_dec(df_275425_pz_rating2, AA_AR_z)
```
...
# REMOVING OUTLIERS 
p3_aaar_model_no_outliers <- lm(formula = AA_AR_z ~ ADI_NATRANK_z + site, data = df_275425_pz_no_outliers)
df_275425_pz_model_no_outliers <- df_275425_pz_no_outliers %>%
  filter(complete.cases(ADI_NATRANK_z) & complete.cases(site)) %>%
  mutate(predicted = predict(p3_aaar_model_no_outliers))

scatterplot_275425_adi_no_outliers <- ggplot(df_275425_pz_model_no_outliers, aes(x=predicted, y=AA_AR_z)) +
  geom_point(colour="black") +
  stat_smooth(method="lm", se=TRUE) +
  theme_apa() +
...
df_275425_missing_na <- df_275425_pz_ema %>%
...
```{r, include=FALSE}
df_50150_fz_ema$na_cubert <- df_50150_fz_ema$na^(1/3)
...
#         zi = ~ ADI_NATRANK_z*AA_AR_z + (1 | ID) + (1 | site),
#   data = df_275425_pz_ema_no_outliers,
#   family=gaussian, REML=TRUE, na.action=na.exclude)
...
```{r}
df_275425_pz_ema_rating <- df_BUDS_rating_it_pt %>%
  select(-site) %>%
  full_join(df_275425_pz_ema, by="ID", relationship="many-to-many")
model_275425_ema_na_dec <- glmmTMB(na_cubert ~ ADI_NATRANK_z*AA_AR_z + SP_debriefing_deceived + (1 | ID) + (1 | site),
        zi = ~ ADI_NATRANK_z*AA_AR_z + (1 | ID) + (1 | site),
  data = df_275425_pz_ema_rating,
  family=gaussian, REML=TRUE, na.action=na.exclude)

df_275425_pz_ema_rating <- df_275425_pz_ema %>%
    select(-site) %>%
