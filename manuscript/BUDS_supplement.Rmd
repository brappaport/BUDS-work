...
### RewP
```{r}
summary(lmer(formula = ERP ~ Feedback*ADI_NATRANK_c + Age + Sex + demo_child_hispanic + White_nonwhite + StressCT + StressTH + IDAS_wellbeing + (1 | ID) + (1 | site), data = filter(df_150275_cz_long_strain, Voting=="Like")))
...
### P3
```{r}
summary(lmer(formula = ERP ~ Feedback*ADI_NATRANK_c + 
               Age + Sex + demo_child_hispanic + White_nonwhite + StressCT + StressTH + IDAS_wellbeing + (1 | ID) + (1 | site), data = filter(df_275425_cz_long_strain, Voting=="Like")))
...
```
The interactions between ADI and Feedback condition predicting mean RewP and P3 amplitude remain significant even when covarying for age, sex, hispanic ethnicity, majority/minority race, stress life event frequency and severity, and self-reported well-being.
...
summary(lmer(formula = ERP ~ Feedback*ADI_NATRANK_c + SP_debriefing_deceived + site.x + (1 | ID), data = filter(df_150275_cz_long_rating, Voting=="Like")))
```
