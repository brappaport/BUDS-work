...
sex_by_site <- table(df_275425_cz_strain$demo_child_sex, df_275425_cz_strain$site)
# Make a new row
new_row <- data.frame(Variable = "Female", 
          "Columbia (N=90)" = paste0(sex_by_site[1,1], " (",round(sex_by_site[1,1]/sum(sex_by_site[,1]),4)*100,"%)"),
          "Northwestern (N=70)" = paste0(sex_by_site[1,2], " (",round(sex_by_site[1,2]/sum(sex_by_site[,2]),4)*100,"%)"),
          Stat = "",
          es = "")
...
demo_table2$Stat <- c(demo_table2$Stat[1:2],"$\\chi^2 = 0.58$","--","$\\chi^2 = 0.43$","$\\chi^2 = 10.57$",demo_table2$Stat[7:15])

demo_table2$Stat <- ifelse(demo_table2$Stat=="--", NA, ifelse(demo_table2$Stat=="-", NA, demo_table2$Stat))
demo_table2$es <- ifelse(demo_table2$es=="--", NA, ifelse(demo_table2$es=="-", NA, demo_table2$es))
demo_table2$`Northwestern (N=70)` <- ifelse(demo_table2$`Northwestern (N=70)`=="-", "0 (0.00%)", demo_table2$`Northwestern (N=70)`)
  
apa_table(demo_table2,
          note=demo_table$caption,
...
```{r, include=TRUE, results="asis", fig.cap = "Grand average ERP for acceptance and rejection feedback from high-value peers. Colored sections of the graph represent time windows where ERP components were extracted at Cz electrode: N1 = purple, RewP = orange, P300 = blue."}
Cz_ga_aa_ar_plot
```
...
```{r, include=TRUE, results="asis", fig.cap = "Mean ratings across all participants by Feedback and Value."}
rating_plot
```
...
ggarrange(Cz_ga_aa_ar_plot, rating_plot)
```
...
```{r, include=TRUE, results="asis", fig.cap = "Relationship between SES and brain responses to acceptance (relative to rejection) for both the RewP and P300 ERP components."}
...
adi_plots_scatter <- annotate_figure(adi_plots_scatter, 
                                   left=text_grob("Mean amplitude (µV; residualized response\n to acceptance relative to rejection)", color = "black", rot = 90, size=16),
                                   bottom=text_grob("SES", color = "black", size=14))

adi_plots_scatter
# ggsave(filename="../figures/adi_plots_scatter.png", plot=adi_plots_scatter,
#        width=9, height = 7)
```
...
plot_150275_adi_na <- ggpredict(model_150275_ema_na, terms = c("AA_AR_z","ADI_NATRANK_z"))%>% 
  plot() + 
  theme_apa() +
  labs(x="RewP to acceptance\n (relative to rejection)", y="Mean negative affect (cubed root)", color="SES", linetype="SES", title="") +
...
plot_275425_adi_na_rej <- ggpredict(model_275425_ema_na_AR, terms = c("AR_AA_z","ADI_NATRANK_z")) %>%
  plot() + 
  theme_apa() +
  labs(x="P300 to rejection\n (relative to acceptance)", y="Mean negative affect (cubed root)", color="SES", linetype="SES", title="") +
...
# m_p300_rej_cov_ss <- emtrends(model_275425_ema_na_AR_cov, pairwise ~ADI_NATRANK_c, var="AR_AA_c",at=SES_list, adjust="none")|> test()
```

```{r, include=TRUE, results='asis', fig.cap = "Interactions for which SES significantly moderated associated between brain responses to acceptance/rejection and negative affect. A: no simple slopes significant. B: Simple slope significant for SES -1 standard deviation below the mean."}
...
adi_plots_na <- ggarrange(plot_150275_adi_na + coord_cartesian(xlim=c(-4,3), ylim=c(1.5,3.5)) + rremove("ylab") + annotate("text", x=0, y=3.5, 
                                                label = paste0("b= ",round(model_150275_ema_na_stats$estimate,3), 
                                                               ", p= ",round(model_150275_ema_na_stats$p.value,3)),
                                                color = "black", rot = 0, size=4)  
                          ...
                          plot_275425_adi_na_rej + coord_cartesian(xlim=c(-3,3), ylim=c(1.5,3.5)) + rremove("ylab") + annotate("text", x=0, y=3.5, 
                                                label = paste0("b= ",round(model_275425_ema_na_AR_pa_stats$estimate,3), 
                                                               ", p= ",round(model_275425_ema_na_AR_pa_stats$p.value,3)),
                                                color = "black", rot = 0, size=4)  
