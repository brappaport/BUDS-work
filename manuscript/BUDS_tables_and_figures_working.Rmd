...
scatterplot_150275_adi <- ggplot(df_150275_cz, aes(x=ADI_NATRANK, y=AA_AR_z)) +
  ...
  ylim(-3,5) +
  theme_apa() +
  labs(x="SES", y=expression("RewP"[resid])) +
  theme()
```

```{r}
p3_aaar_model <- ses_lm(df_275425_cz_strain, AA_AR_z)
p3_araa_model <- ses_lm(df_275425_cz_strain, AR_AA_z)
p3_rarr_model <- ses_lm(df_275425_cz_strain, RA_RR)
p3_rrra_model <- ses_lm(df_275425_cz_strain, RR_RA)
...
scatterplot_275425_adi <- ggplot(df_275425_cz, aes(x=ADI_NATRANK, y=AA_AR_z)) +
  ...
  stat_smooth(method="lm", se=TRUE) +
  theme_apa() +
  labs(x="SES", y=expression("Acceptance P300"[resid])) +
  theme()
```
...
cowplot::plot_grid(adi_plots_scatter, adi_plots_na,
          labels = c("A","B"),
          ncol = 2, nrow = 1,
          rel_heights = c(1,1),
          rel_width=c(2.5,3),
...
scatterplot_275425_disc_no_outliers <- ggplot(df_275425_cz_model, aes(x=predicted, y=AA_AR_z)) +
  ...
  stat_smooth(method="lm", se=TRUE) +
  theme_apa() +
  labs(x="Discrimination (covarying for site)", y=expression("Acceptance P300"[resid])) +
  theme()
```

```{r, include=TRUE, results='asis', fig.cap="Relationship between SES and brain responses to acceptance (relative to rejection) for both the RewP and P300 ERP components.", fig.width=5, fig.height=5}
